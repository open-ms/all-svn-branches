...
MSMetaDataExplorer::MSMetaDataExplorer(QWidget *parent, const char *name) : Q3MainWindow(parent, name)
{
  ...
  //Create the tree for exploring data 
	listview_ = new Q3ListView(split_);    
  ...
  connect(listview_, SIGNAL(clicked(Q3ListViewItem*)), this, SLOT(showDetails(Q3ListViewItem*))  );
	//save all changes
...
void MSMetaDataExplorer::showDetails(Q3ListViewItem *item)
{
...
void MSMetaDataExplorer::visualize_(Sample &s, Q3ListViewItem* parent )
{
    ...
    String w_id(widgetID);
		if(parent == 0)
		{
			item = new Q3ListViewItem(listview_, listview_->lastItem(), "Sample" , w_id);
		}
		else
		{
			item = new Q3ListViewItem(parent, "Sample" , w_id);
		} 
...
void MSMetaDataExplorer::visualize_(MetaInfoInterface &m, Q3ListViewItem* parent)
{
		...
		if(parent==0)
		{
			new Q3ListViewItem(listview_, listview_->lastItem(), "MetaInfo" , w_id);
		}
		else
		{
			new Q3ListViewItem(parent, "MetaInfo" , w_id);
		}
...
void MSMetaDataExplorer::visualize_(Digestion &d, Q3ListViewItem* parent)
{
    Q3ListViewItem* item;
		...
		if(parent==0)
		{
			item =new Q3ListViewItem(listview_, listview_->lastItem(), "Digestion" , w_id);
			 	
		}
		else
		{
			item = new Q3ListViewItem(parent, "Digestion" , w_id);
		}
...
void MSMetaDataExplorer::visualize_(Modification &m, Q3ListViewItem* parent)
{
    ...
    int widgetID = makeID();
		...
		if(parent==0)
		{
			item =new Q3ListViewItem(listview_, listview_->lastItem(), "Modification" , w_id);
		}
		else
		{
			item = new Q3ListViewItem(parent, "Modification" , w_id);
		}
		//check for metainfo objects
		if(! m.isMetaEmpty() )
		{
...
void MSMetaDataExplorer::visualize_(Tagging &t, Q3ListViewItem* parent)
{
    ...
    int widgetID = makeID();
		...
		if(parent==0)
		{
			item =new Q3ListViewItem(listview_, listview_->lastItem(), "Tagging" , w_id);
		}
		else
		{
			item =new Q3ListViewItem(parent, "Tagging" , w_id);  
		}
		//check for metainfo objects
		if(! t.isMetaEmpty() )
		{
...
void MSMetaDataExplorer::visualize_(Software &s, Q3ListViewItem* parent)
{
    ...
    int widgetID = makeID();
		...
		if(parent==0)
		{
			item = new Q3ListViewItem(listview_, listview_->lastItem(), "Software" , w_id);
		}
		else
		{
			item = new Q3ListViewItem(parent, "Software" , w_id);  
		}
...
void MSMetaDataExplorer::visualize_(HPLC &h, Q3ListViewItem* parent)
{
    ...
    int widgetID = makeID();
		...
		if(parent==0)
		{
			item = new Q3ListViewItem(listview_, listview_->lastItem(), "HPLC" , w_id);
		}
		else
		{
			item = new Q3ListViewItem(parent, "HPLC" , w_id);  
		}
...
void MSMetaDataExplorer::visualize_(Gradient &gradient, Q3ListViewItem* parent)
{
    int widgetID = makeID();
		...
		if(parent==0)
		{
			new Q3ListViewItem(listview_, listview_->lastItem(), "Gradient" , w_id);
		}
		else
		{
			new Q3ListViewItem(parent, "Gradient" , w_id);  
		}
...
void MSMetaDataExplorer::visualize_(SourceFile &source, Q3ListViewItem* parent)
{
    int widgetID = makeID();
		...
		if(parent==0)
		{
			new Q3ListViewItem(listview_, listview_->lastItem(), "SourceFile" , w_id);
		}
		else
		{
			new Q3ListViewItem(parent, "SourceFile" , w_id);  
		}
