...
private:
	...
	void handleParameters()
	{
		...
		all_pairs.setExperimentType("peptide");
...
//		mz_stepwidth=0.0005;
		...
		{
				filters.push_back(SILACFilter(charge,model_deviation));
		}
...
public:
	TOPPPeptideAnalyzer()
	: TOPPBase("PeptideAnalyzer","Determination of peak ratios in LC-MS data",true)
	{
	}
	...
	ExitCodes main_(int , const char**)
	{
		...
		{
			std::cout << data_it->size() << std::endl;
						if (data_it->size() < 2)
							continue;
