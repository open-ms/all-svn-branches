	...
	: public TOPPBase
	  {
				...
				const vector<UInt64> samples)
		{
					...
					++q_it)
			{
				...
				//write protein abundances/intensities
				for (vector<UInt64>::const_iterator samp_it = samples.begin();
						samp_it != samples.end(); ++samp_it)
				{
				...
				// write ratios
				UInt64 ref_abundances = sample.front();
				for (vector<UInt64>::const_iterator samp_it = samples.begin();
						samp_it != samples.end(); ++samp_it)
				{
		...
		void writeComments_(SVOutStream& out, const ConsensusMap::FileDescriptions files, const bool proteins=true)
		{
