...
void HashClustering::init()
{
	...
	for (GridElements::iterator it=hashing_map.begin();it!=hashing_map.end();++it)
	{
		...
		{
			if (i<0 || i>grid.grid_size_x)
				continue;
			for (int j=y;j<=y+1;++j)
			{
				if (j<0 || j>grid.grid_size_y || (i==x-1 && j==y))
					continue;
				...
				for (std::list<GridElement*>::iterator lit=elements.begin();lit!=elements.end();++lit)
				{
					for (std::list<GridElement*>::iterator nit=neighbor_elements.begin();nit!=neighbor_elements.end();++nit)
					{
						...
						std::pair<DistanceSet::iterator,bool> position=grid.distances.insert(DistanceEntry(element_ptr,neighbor_ptr,act_distance));
						if (position.second)
						{
...
typedef std::map<std::pair<int,int>, std::list<GridElement*> > ElementMap;
	...
	void HashClustering::merge(DataSubset& subset1,DataSubset& subset2)
	{
			...
			for (IteratorMap::iterator dist_it=subset1.distance_iterators.begin();dist_it!=subset1.distance_iterators.end();)
			{
				...
				{
					grid.distances.erase(dist_it->second);
					subset1.distance_iterators.erase(dist_it++);
				}
				else
				{
					entry.distance=new_distance;
					grid.distances.replace(dist_it->second,entry);
					++dist_it;
				}
			}
			for (IteratorMap::iterator dist_it=subset2.distance_iterators.begin();dist_it!=subset2.distance_iterators.end();++dist_it)
				{
					...
					else
					{
						entry.owner=&subset1;
						entry.distance=new_distance;
						bool replaced=grid.distances.replace(dist_it->second,entry);
						if (replaced)
			...
			if (x!=x_new || y!=y_new)
			{
				...
				for (int i=x_new-1;i<=x_new+1;++i)
				{
					...
					{
						if (j<0 || j>grid.grid_size_y || (i==x_new+1 && j==y_new))
							continue;
			...
			for (std::set<std::pair<int,int> >::iterator set_it=traverse_set.begin();set_it!=traverse_set.end();++set_it)
			{
				...
				for (std::list<GridElement*>::iterator lit=neighbor_elements.begin();lit!=neighbor_elements.end();++lit)
				{
					...
					{
						grid.distances.erase(pos->second);
					...
					else
					{
						if (pos!=neighbor_ptr->distance_iterators.end())
						{
							grid.distances.replace(pos->second,DistanceEntry(neighbor_ptr,&subset1,act_distance));
						}
						else
						{
							std::pair<DistanceSet::iterator,bool> new_pos=grid.distances.insert(DistanceEntry(neighbor_ptr,&subset1,act_distance));
	...
	{
		DistanceSet::index<Dist>::type& dist_elements=grid.distances.get<Dist>();
	...
	{
		Int distance_size=grid.distances.size();
		...
		do
		{
			...
			setProgress(distance_size - grid.distances.size());
		}
		while(grid.distanceSize() > 0);
		endProgress();
