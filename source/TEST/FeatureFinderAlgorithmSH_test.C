...
FeatureFinderAlgorithmSH<Peak1D, Feature>* ptr = 0;
...
START_SECTION((virtual void run()))
{
  ...
  ff.setLogType(ProgressLogger::NONE);
	...
	ff.run("superhirn", exp, features, ff_param, seeds);
...
START_SECTION((static FeatureFinderAlgorithm<PeakType,FeatureType>* create()))
{
  FeatureFinderAlgorithm<Peak1D, Feature>* ptr2 = 0;
	ptr2 = FeatureFinderAlgorithmSH<Peak1D, Feature>::create();
...
{
  TEST_STRING_EQUAL(ptr->getProductName(), "superhirn")
}
END_SECTION
